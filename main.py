...
def get_items(html, items):
    ...
    item_divs = soup.find_all('div', class_='catalog-items-desktop')
    if len(item_divs) == 0:
        return False
    for item in item_divs:
        link = BASEURL + item.find('a', class_='ddl_product_link').get('href')
        item_price = item.find('div', class_='catalog-item-regular-desktop__price-block')
        if item_price:
            ...
            if item_bonus:
                ...
                item_title = item.find('div', class_='catalog-item-regular-desktop__title-link').get_text()
